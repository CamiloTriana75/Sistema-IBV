...
const handleManualInput = () => {
  if (manualInput.value.trim()) {
    ...
    setTimeout(() => { justScanned.value = false }, 1500)
    emit('scan', val)
    manualInput.value = ''
  }
}
...
<template>
  <div class="space-y-4">
    ...
    <div v-if="showCamera" class="relative">
      <div class="bg-gray-900 rounded-2xl overflow-hidden aspect-[4/3] flex items-center justify-center relative">
        <!-- Visor de cámara simulado -->
        <div class="absolute inset-0 flex items-center justify-center">
          <div class="w-56 h-56 relative">
            <!-- Esquinas del marco de escaneo -->
            <div class="absolute top-0 left-0 w-8 h-8 border-t-4 border-l-4 border-primary-400 rounded-tl-lg" />
            <div class="absolute top-0 right-0 w-8 h-8 border-t-4 border-r-4 border-primary-400 rounded-tr-lg" />
            <div class="absolute bottom-0 left-0 w-8 h-8 border-b-4 border-l-4 border-primary-400 rounded-bl-lg" />
            <div class="absolute bottom-0 right-0 w-8 h-8 border-b-4 border-r-4 border-primary-400 rounded-br-lg" />
            <!-- Línea de escaneo animada -->
            <div class="absolute left-2 right-2 h-0.5 bg-primary-400 shadow-lg shadow-primary-400/50 animate-scan-line" />
          </div>
        </div>
        <!-- Ícono central -->
...
class="inline-flex items-center gap-2 px-4 py-2.5 text-sm font-semibold rounded-xl transition"
        ...
        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          <path v-if="!showCamera" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 9a2 2 0 012-2h.93a2 2 0 001.664-.89l.812-1.22A2 2 0 0110.07 4h3.86a2 2 0 011.664.89l.812 1.22A2 2 0 0018.07 7H19a2 2 0 012 2v9a2 2 0 01-2 2H5a2 2 0 01-2-2V9z" />
          <circle v-if="!showCamera" cx="12" cy="13" r="3" stroke="currentColor" stroke-width="2" />
          <path v-if="showCamera" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12" />
        </svg>
    ...
    <div class="relative">
      ...
      />
      <button
:disabled="!manualInput.trim()" class="absolute right-2 top-1/2 -translate-y-1/2 px-4 py-1.5 bg-primary-600 text-white text-sm font-semibold rounded-lg hover:bg-primary-700 transition disabled:opacity-40 disabled:cursor-not-allowed"
    ...
    <Transition enter-active-class="transition duration-200 ease-out" enter-from-class="translate-y-1 opacity-0" enter-to-class="translate-y-0 opacity-100">
      <div v-if="scannedValue && !hideResult" class="flex items-center gap-3 p-3 bg-green-50 border border-green-200 rounded-xl">
