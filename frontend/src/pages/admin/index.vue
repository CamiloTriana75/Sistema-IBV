<template>
  <div>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6 mb-8">
      <div v-for="stat in stats" :key="stat.label" class="bg-white rounded-xl p-5 shadow-sm border border-gray-100 hover:shadow-md transition-shadow">
        <div class="flex items-center justify-between">
          <div>
            <p class="text-sm text-gray-500 font-medium">{{ stat.label }}</p>
            <p class="text-2xl font-bold text-gray-900 mt-1">{{ stat.value }}</p>
            <p :class="['text-xs mt-1 font-medium', stat.changePositive ? 'text-success-500' : 'text-danger-500']">
              {{ stat.change }}
            </p>
          </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-8">
      ...
      <div class="lg:col-span-2 bg-white rounded-xl shadow-sm border border-gray-100">
        <div class="px-6 py-4 border-b border-gray-100 flex items-center justify-between">
          ...
          <button class="text-sm text-primary-600 hover:text-primary-700 font-medium">Ver todo</button>
        </div>
      ...
      <div class="bg-white rounded-xl shadow-sm border border-gray-100">
        ...
        <div class="p-4 space-y-2">
          ...
          >
            <div class="w-10 h-10 bg-primary-100 text-primary-600 rounded-lg flex items-center justify-center group-hover:bg-primary-200 transition">
          ...
          <NuxtLink
            ...
            class="flex items-center gap-3 p-3 rounded-lg hover:bg-primary-50 transition group"
          >
            <div class="w-10 h-10 bg-warning-500/10 text-warning-600 rounded-lg flex items-center justify-center group-hover:bg-warning-500/20 transition">
              <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          <div class="flex items-center gap-3 p-3 rounded-lg hover:bg-primary-50 transition group cursor-pointer">
            <div class="w-10 h-10 bg-success-500/10 text-success-600 rounded-lg flex items-center justify-center group-hover:bg-success-500/20 transition">
              <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          <div class="flex items-center gap-3 p-3 rounded-lg hover:bg-primary-50 transition group cursor-pointer">
            <div class="w-10 h-10 bg-danger-500/10 text-danger-600 rounded-lg flex items-center justify-center group-hover:bg-danger-500/20 transition">
              <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-100">
      ...
      <div class="p-6">
        <div class="grid grid-cols-2 sm:grid-cols-5 gap-4">
          <div v-for="status in vehicleStatuses" :key="status.label" class="text-center p-4 rounded-xl bg-gray-50">
            ...
            <div class="mt-2 h-1.5 bg-gray-200 rounded-full overflow-hidden">
              <div :class="['h-full rounded-full', status.barColor]" :style="{ width: status.percent + '%' }" />
            </div>
          </div>
        </div>
      </div>
