...
const dañoDetalles = reactive<Record<string, { tipo: string; severidad: 'Baja' | 'Media' | 'Alta' }>>({})
  ...
  <div>
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
      ...
      <div class="flex gap-3">
        <NuxtLink
v-if="isEditing" :to="`/recibidor/impronta-print?id=${editingId}`" target="_blank"
        ...
        <button
:disabled="saving" class="inline-flex items-center gap-2 px-5 py-2.5 bg-primary-600 text-white text-sm font-semibold rounded-xl hover:bg-primary-700 transition shadow-lg shadow-primary-500/25 disabled:opacity-50 disabled:cursor-not-allowed"
      ...
      <div class="xl:col-span-2 space-y-6">
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
          ...
          <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            <div>
              <label class="block text-xs font-semibold text-gray-500 uppercase tracking-wider mb-1.5">VIN / Chasis <span class="text-red-500">*</span></label>
              <input
v-model="form.vin" type="text" placeholder="1HGBH41JXMN109186"
            ...
            <div>
              ...
              <input
v-model="form.placa" type="text" placeholder="ABC-1234"
            ...
            <div>
              <label class="block text-xs font-semibold text-gray-500 uppercase tracking-wider mb-1.5">Marca <span class="text-red-500">*</span></label>
              <input
v-model="form.marca" type="text" placeholder="Toyota"
                class="w-full px-3 py-2.5 border rounded-xl text-sm focus:ring-2 focus:ring-primary-500 focus:border-primary-500 transition"
                :class="errors.marca ? 'border-red-400' : 'border-gray-200'" />
              <p v-if="errors.marca" class="text-xs text-red-500 mt-1">{{ errors.marca }}</p>
            </div>
            <div>
              <label class="block text-xs font-semibold text-gray-500 uppercase tracking-wider mb-1.5">Modelo <span class="text-red-500">*</span></label>
              <input
v-model="form.modelo" type="text" placeholder="Corolla"
                class="w-full px-3 py-2.5 border rounded-xl text-sm focus:ring-2 focus:ring-primary-500 focus:border-primary-500 transition"
            ...
            <div>
              ...
              <input
v-model="form.anio" type="number" placeholder="2024"
                class="w-full px-3 py-2.5 border border-gray-200 rounded-xl text-sm focus:ring-2 focus:ring-primary-500 focus:border-primary-500 transition" />
            </div>
            <div>
              <label class="block text-xs font-semibold text-gray-500 uppercase tracking-wider mb-1.5">Color</label>
              <input
v-model="form.color" type="text" placeholder="Blanco Perla"
                class="w-full px-3 py-2.5 border border-gray-200 rounded-xl text-sm focus:ring-2 focus:ring-primary-500 focus:border-primary-500 transition" />
            </div>
            <div>
              <label class="block text-xs font-semibold text-gray-500 uppercase tracking-wider mb-1.5">Kilometraje</label>
              <input
v-model="form.km" type="number" placeholder="0"
                class="w-full px-3 py-2.5 border border-gray-200 rounded-xl text-sm focus:ring-2 focus:ring-primary-500 focus:border-primary-500 transition" />
            </div>
            <div>
              <label class="block text-xs font-semibold text-gray-500 uppercase tracking-wider mb-1.5">Cliente</label>
              <input
v-model="form.cliente" type="text" placeholder="Distribuidora XXX"
                class="w-full px-3 py-2.5 border border-gray-200 rounded-xl text-sm focus:ring-2 focus:ring-primary-500 focus:border-primary-500 transition" />
            </div>
            <div>
              <label class="block text-xs font-semibold text-gray-500 uppercase tracking-wider mb-1.5">Condición General <span class="text-red-500">*</span></label>
              <select
v-model="form.condicion"
              ...
              <p v-if="errors.condicion" class="text-xs text-red-500 mt-1">{{ errors.condicion }}</p>
            </div>
          </div>
        </div>
...
x="95" y="155" width="130" height="115" rx="8" :fill="zonasDañadas.includes('techo') ? '#fca5a5' : 'transparent'"
            ...
            <div class="flex-1 space-y-3">
              ...
              <div v-for="zona in zonasDañadas" :key="zona" class="p-3 bg-danger-50 border border-danger-100 rounded-xl space-y-2">
                <div class="flex items-center gap-3">
                  ...
                  <button class="text-danger-400 hover:text-danger-600 transition" @click="toggleZona(zona)">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <div class="flex gap-2">
                  <select v-model="dañoDetalles[zona].tipo" class="flex-1 text-xs px-2 py-1.5 border border-gray-200 rounded-lg bg-white">
                  ...
                  <select v-model="dañoDetalles[zona].severidad" class="text-xs px-2 py-1.5 border border-gray-200 rounded-lg bg-white">
              ...
              <div class="mt-4">
                ...
                <textarea
v-model="form.observaciones" rows="4" placeholder="Describa los daños encontrados, rayones, abolladuras, faltantes, etc."
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
          ...
          <div class="space-y-3">
            ...
            <div class="grid grid-cols-2 gap-3">
              <div
v-for="vista in vistasRequeridas" :key="vista.key"
...
class="absolute top-1.5 left-1.5 w-6 h-6 bg-danger-500 rounded-full items-center justify-center hidden group-hover:flex"
          ...
          <div class="mt-4 space-y-3">
            ...
            <div class="grid grid-cols-3 gap-2">
              <div
v-for="(foto, idx) in fotosAdicionales" :key="idx"
...
enter-active-class="transition duration-300 ease-out" enter-from-class="translate-y-2 opacity-0"
      ...
      <div
v-if="toastMsg" class="fixed bottom-6 right-6 z-50 flex items-center gap-3 px-5 py-4 rounded-2xl shadow-2xl text-white font-semibold"
        :class="toastType === 'error' ? 'bg-red-600' : 'bg-success-600'">
        <svg v-if="toastType === 'error'" class="w-5 h-5 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z" />
        </svg>
