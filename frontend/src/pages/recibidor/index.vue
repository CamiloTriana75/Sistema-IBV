<template>
  <div>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
      <div class="bg-white rounded-xl p-5 shadow-sm border border-gray-100">
        <div class="flex items-center justify-between mb-2">
          ...
          <span class="text-xs font-semibold text-primary-600 bg-primary-50 px-2 py-0.5 rounded-full">Total</span>
        </div>
      ...
      <div class="bg-white rounded-xl p-5 shadow-sm border border-gray-100">
        <div class="flex items-center justify-between mb-2">
          ...
          <span class="text-xs font-semibold text-amber-600 bg-amber-50 px-2 py-0.5 rounded-full">Pendientes</span>
        </div>
      ...
      <div class="bg-white rounded-xl p-5 shadow-sm border border-gray-100">
        <div class="flex items-center justify-between mb-2">
          ...
          <span class="text-xs font-semibold text-purple-600 bg-purple-50 px-2 py-0.5 rounded-full">Revisadas</span>
        </div>
    ...
    <div v-if="contStore.pendientes + contStore.enRecepcion > 0" class="bg-gradient-to-r from-primary-50 to-blue-50 rounded-xl border border-primary-100 p-5 mb-6">
      <div class="flex items-center justify-between mb-3">
        <div class="flex items-center gap-3">
          ...
          <div>
            ...
            <p class="text-xs text-gray-500">{{ contStore.pendientes }} pendientes · {{ contStore.enRecepcion }} en recepción</p>
          </div>
        </div>
        <NuxtLink to="/recibidor/escaneo"
      ...
      <div class="flex gap-2 flex-wrap">
        <span v-for="cont in contStore.contenedores.filter(c => c.estado !== 'completado')" :key="cont.id"
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-4 mb-6">
      <div class="flex flex-col sm:flex-row gap-3">
        <div class="flex-1 relative">
          ...
          <input v-model="search" type="text" placeholder="Buscar por folio, VIN, placa, marca..."
        ...
        <select v-model="filterCondicion"
          class="px-4 py-2.5 border border-gray-200 rounded-lg text-sm bg-white focus:ring-2 focus:ring-primary-500">
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
        ...
        <table class="w-full">
          <thead>
            <tr class="bg-gray-50 border-b border-gray-100">
              <th class="px-5 py-3.5 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">Folio</th>
              <th class="px-5 py-3.5 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">Vehículo</th>
              <th class="px-5 py-3.5 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">Cliente</th>
              <th class="px-5 py-3.5 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">Condición</th>
          ...
          <tbody class="divide-y divide-gray-50">
            <tr v-for="imp in filteredImprontas" :key="imp.id" class="hover:bg-gray-50/50 transition">
              <td class="px-5 py-4">
                <span class="text-sm font-bold text-primary-600 font-mono">{{ imp.folio }}</span>
              </td>
              <td class="px-5 py-4">
                <div>
                  <p class="text-sm font-semibold text-gray-900">{{ imp.marca }} {{ imp.modelo }} {{ imp.anio }}</p>
                  <p class="text-xs text-gray-500 font-mono">{{ imp.placa || 'Sin placa' }} · {{ imp.vin ? imp.vin.slice(-8) : 'S/N' }}</p>
                </div>
              </td>
              <td class="px-5 py-4">
              ...
              <td class="px-5 py-4">
                <span :class="condicionBadge(imp.condicion)" class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-semibold">
                  <span :class="condicionDot(imp.condicion)" class="w-1.5 h-1.5 rounded-full" />
                  {{ condicionLabel(imp.condicion) }}
                </span>
              </td>
              <td class="px-5 py-4">
                <span :class="estadoBadge(imp.estado)" class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-semibold">
              ...
              <td class="px-5 py-4">
                <div class="flex items-center justify-center gap-2">
                  <NuxtLink :to="`/recibidor/impronta?id=${imp.id}`"
                  ...
                  <NuxtLink :to="`/recibidor/impronta-print?id=${imp.id}`" target="_blank"
                  ...
                  <button @click="confirmarEliminar(imp)"
      ...
      <div v-if="filteredImprontas.length > 0" class="px-5 py-3 bg-gray-50 border-t border-gray-100 text-xs text-gray-500">
        ...
        <div v-if="deleteTarget" class="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div class="relative bg-white rounded-2xl shadow-2xl w-full max-w-sm p-6 text-center">
            ...
            <p class="text-sm text-gray-500 mb-6">
              ¿Eliminar <span class="font-semibold text-gray-700">{{ deleteTarget.folio }}</span> ({{ deleteTarget.marca }} {{ deleteTarget.modelo }})? Esta acción no se puede deshacer.
            </p>
            <div class="flex gap-3">
    ...
    <Transition enter-active-class="transition duration-300 ease-out" enter-from-class="translate-y-2 opacity-0"
      ...
      <div v-if="toast" class="fixed bottom-6 right-6 z-50 flex items-center gap-3 px-5 py-3 rounded-xl shadow-lg text-white font-medium bg-green-600">
...
const condicionLabel = (c: string) => ({ excelente: 'Excelente', bueno: 'Bueno', regular: 'Regular', dañado: 'Con daños' }[c] || '—')
...
const estadoLabel = (e: string) => ({ borrador: 'Borrador', completada: 'Completada', revisada: 'Revisada' }[e] || e)
