  ...
  <div>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-4 gap-4 mb-8">
      ...
      <div class="bg-white rounded-xl p-5 shadow-sm border border-gray-100">
        ...
        <p class="text-3xl font-bold text-warning-600 mt-1">{{ vehiculoStore.pendientesInventario }}</p>
      </div>
      <div class="bg-white rounded-xl p-5 shadow-sm border border-gray-100">
    ...
    <div v-if="vehiculoStore.listosDespacho > 0" class="bg-success-50 border border-success-200 rounded-xl p-4 mb-6 flex items-center gap-4">
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-100">
      ...
      <div v-if="vehiculoStore.getListosParaDespacho.length === 0" class="px-6 py-12 text-center">
        ...
        <p class="text-gray-300 text-sm mt-1">Los vehículos necesitan impronta completada e inventario aprobado</p>
      </div>
      <div v-else class="overflow-x-auto">
        <table class="w-full">
          <thead class="bg-gray-50 border-b border-gray-100">
            <tr>
              <th class="px-6 py-3 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">VIN</th>
              ...
              <th class="px-6 py-3 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">Impronta</th>
              <th class="px-6 py-3 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">Inventario</th>
              <th class="px-6 py-3 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">Estado</th>
            </tr>
          </thead>
          <tbody class="divide-y divide-gray-50">
            <tr v-for="v in vehiculoStore.getListosParaDespacho" :key="v.id" class="hover:bg-gray-50 transition">
              <td class="px-6 py-4 text-sm font-mono font-medium text-gray-900">{{ v.vin.slice(-8) }}</td>
              <td class="px-6 py-4 text-sm text-gray-700">{{ v.marca }} {{ v.modelo }} {{ v.anio }}</td>
              <td class="px-6 py-4 text-sm text-gray-600">{{ v.cliente || '—' }}</td>
              <td class="px-6 py-4">
                <span class="px-2 py-1 text-xs font-semibold bg-blue-100 text-blue-700 rounded-full">{{ v.improntaFolio || '✓' }}</span>
              </td>
              <td class="px-6 py-4">
                <span class="px-2 py-1 text-xs font-semibold bg-success-100 text-success-700 rounded-full">
                  {{ v.inventarioResultado ? `${v.inventarioResultado.aprobados}/${v.inventarioResultado.totalItems}` : '✓' }}
                </span>
              </td>
              <td class="px-6 py-4">
                <span class="px-2.5 py-1 text-xs font-semibold bg-success-500/10 text-success-600 rounded-full">
                  Listo
                </span>
              </td>
            </tr>
    ...
    <div v-if="vehiculosBloqueados.length > 0" class="mt-6 bg-white rounded-xl shadow-sm border border-gray-100">
      ...
      <div class="divide-y divide-gray-50">
        <div v-for="v in vehiculosBloqueados" :key="v.id"
          ...
          <div class="flex items-center gap-4">
            ...
            <div>
              <p class="text-sm font-bold text-gray-900">{{ v.marca }} {{ v.modelo }} {{ v.anio }}</p>
