<template>
  <div>
    ...
    <div class="bg-gradient-to-br from-primary-600 to-primary-700 rounded-2xl p-8 text-white mb-8 shadow-lg">
      <div class="flex flex-col sm:flex-row items-center gap-6">
        ...
        <div class="text-center sm:text-left flex-1">
          ...
          <p class="text-primary-100 mt-1">Verifica la autorización de salida/entrada de vehículos</p>
        </div>
        <button class="px-6 py-3 bg-white text-primary-600 font-semibold rounded-xl hover:bg-primary-50 transition shadow-lg">
          Abrir Escáner
        </button>
      </div>
    </div>
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-100">
      ...
      <div class="divide-y divide-gray-50">
        <div v-for="mov in movements" :key="mov.id" class="px-6 py-4 flex items-center justify-between hover:bg-gray-50 transition">
          <div class="flex items-center gap-4">
            <div :class="['w-10 h-10 rounded-full flex items-center justify-center', mov.type === 'entrada' ? 'bg-success-500/10 text-success-600' : 'bg-primary-100 text-primary-600']">
              <svg v-if="mov.type === 'entrada'" class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <div>
              ...
              <p class="text-xs text-gray-500">{{ mov.type === 'entrada' ? 'Entrada' : 'Salida' }} · {{ mov.driver }}</p>
            </div>
          </div>
