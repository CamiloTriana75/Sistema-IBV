...
const INITIAL_CONTENEDORES: Contenedor[] = [
  {
    ...
    vehiculos: [
      { vin: '1HGBH41JXMN109186', marca: 'Toyota', modelo: 'Corolla', anio: '2024', color: 'Blanco Perla', codigoImpronta: 'IMP-VH-001', escaneado: false },
      { vin: '3VWDX7AJ5BM123456', marca: 'Toyota', modelo: 'Hilux', anio: '2024', color: 'Gris Oscuro', codigoImpronta: 'IMP-VH-002', escaneado: false },
      { vin: 'JTDKN3DU5A0123789', marca: 'Toyota', modelo: 'Yaris', anio: '2025', color: 'Rojo', codigoImpronta: 'IMP-VH-003', escaneado: false },
    ],
    estado: 'pendiente',
    recibidoPor: '',
    observaciones: '',
  },
  {
    ...
    vehiculos: [
      { vin: 'WBA3A5G59DNP12345', marca: 'Chevrolet', modelo: 'Spark', anio: '2023', color: 'Azul Eléctrico', codigoImpronta: 'IMP-VH-004', escaneado: false },
      { vin: '5YJSA1DNXDFP67890', marca: 'Chevrolet', modelo: 'Aveo', anio: '2024', color: 'Negro', codigoImpronta: 'IMP-VH-005', escaneado: false },
    ],
    estado: 'pendiente',
    recibidoPor: '',
    observaciones: '',
  },
  {
    ...
    vehiculosEsperados: 2,
    vehiculos: [
      { vin: '1FADP3F29JL234567', marca: 'Ford', modelo: 'Explorer', anio: '2025', color: 'Blanco Oxford', codigoImpronta: 'IMP-VH-006', escaneado: true, improntaId: '1' },
      { vin: '3FA6P0HD7LR890123', marca: 'Ford', modelo: 'Escape', anio: '2024', color: 'Plata Estelar', codigoImpronta: 'IMP-VH-007', escaneado: true, improntaId: '2' },
    ],
    estado: 'completado',
...
export const useContenedorStore = defineStore('contenedor', () => {
  ...
  const pendientes = computed(() => contenedores.value.filter(c => c.estado === 'pendiente').length)
  const enRecepcion = computed(() => contenedores.value.filter(c => c.estado === 'en_recepcion').length)
  const completados = computed(() => contenedores.value.filter(c => c.estado === 'completado').length)
  ...
  const buscarVehiculoPorCodigo = (codigoImpronta: string): { contenedor: Contenedor; vehiculo: VehiculoContenedor } | undefined => {
    for (const cont of contenedores.value) {
      const veh = cont.vehiculos.find(v => v.codigoImpronta.toLowerCase() === codigoImpronta.toLowerCase())
      if (veh) return { contenedor: cont, vehiculo: veh }
    }
    return undefined
  }

  const buscarVehiculoPorVin = (vin: string): { contenedor: Contenedor; vehiculo: VehiculoContenedor } | undefined => {
    for (const cont of contenedores.value) {
  ...
  const marcarVehiculoEscaneado = (contenedorId: string, codigoImpronta: string, improntaId?: string) => {
