...
interface VehiculoRow {
  ...
  improntas?: Array<{ id: number; estado: string | null; fecha: string | null }>
  inventarios?: Array<{
    id: number
    completo: boolean
    fecha: string | null
...
export const useInventarioStore = defineStore('inventario', () => {
  ...
  const mapVehiculo = (row: VehiculoRow): InventarioVehiculo => {
    ...
    const improntaCompletada = improntas.some((i) => isImprontaCompletada(i.estado))
    ...
    const vehiculoBase: Omit<InventarioVehiculo, 'estado'> = {
      ...
      placa: '',
      marca: row.modelo?.marca || '—',
      modelo: row.modelo?.modelo || '',
      anio: row.modelo?.anio ? String(row.modelo.anio) : '',
      color: row.color || '—',
      cliente: '',
    ...
    try {
      ...
      const { data, error: fetchError } = await $supabase
        ...
        .select(
          ...
          improntas:improntas(id, estado, fecha),
          inventarios:inventarios(id, completo, fecha, checklist_json, usuario:users_user!inventarios_usuario_id_fkey(nombres, apellidos))
        `,
        )
      ...
      const list = (data || []) as VehiculoRow[]
