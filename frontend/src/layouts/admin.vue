...
const currentUser = computed(() => {
  ...
  return {
    name: u.name || 'Usuario',
    initials: u.avatar || u.name?.split(' ').map((w: string) => w[0]).join('').substring(0, 2) || 'U',
    role: u.role || 'admin',
...
const menuItems = computed(() => {
  ...
  const allMenus: Record<string, Array<{ to: string; label: string; icon: string; badge?: number }>> = {
    admin: [
    ...
    recibidor: [
      ...
      { to: '/recibidor/escaneo', label: 'Escaneo', icon: '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M12 4v1m6 11h2m-6 0h-2v4m0-11v3m0 0h.01M12 12h4.01M16 20h4M4 12h4m12 0h.01M5 8h2a1 1 0 001-1V5a1 1 0 00-1-1H5a1 1 0 00-1 1v2a1 1 0 001 1zm12 0h2a1 1 0 001-1V5a1 1 0 00-1-1h-2a1 1 0 00-1 1v2a1 1 0 001 1zM5 20h2a1 1 0 001-1v-2a1 1 0 00-1-1H5a1 1 0 00-1 1v2a1 1 0 001 1z" /></svg>' },
      { to: '/recibidor/impronta', label: 'Nueva Impronta', icon: '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M12 4v16m8-8H4" /></svg>' },
    ],
    inventario: [
    ...
    despachador: [
      ...
      { to: '/despachador/escaneo', label: 'Escaneo de Lote', icon: '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M13 7l5 5m0 0l-5 5m5-5H6" /></svg>', badge: '2' },
    ],
    porteria: [
